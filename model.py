...
class BaseModel(object):
    ...
    def build_model(self, name, num_outputs, shape):
        ...
        with tf.variable_scope('behaviour'):
            ...
            self.b_fc1, self.behaviour_weights['fc1_w'], self.behaviour_weights['fc1_b'] = linear(self.b_conv3_flat,
                                                                                                  512,
                                                                                                  activation_fn=activation_fn,
                                                                                                  name='b_fc1')
            self.b_out, self.behaviour_weights['out_w'], self.behaviour_weights['out_b'] = linear(self.b_fc1,
                                                                                                  num_outputs,
                                                                                                  name='b_out')
        ...
        with tf.variable_scope('copy'):
            ...
            for name in self.behaviour_weights.keys():
                self.copy_from[name] = tf.placeholder('float32', self.target_weights[name].get_shape().as_list(),
                                                      name=name)
