    ...
    def build_model(self,name,num_outputs,shape):
        ...
        with tf.variable_scope('behaviour'):
            ...
            self.b_conv1, self.behaviour_weights['conv1_w'], self.behaviour_weights['conv1_b'] = conv2d(self.b_x,
                                        32, [8, 8], [4, 4], initializer, activation_fn, name='b_conv1')
            self.b_conv2, self.behaviour_weights['conv2_w'], self.behaviour_weights['conv2_b'] = conv2d(self.b_conv1,
                                        64, [4, 4], [2, 2], initializer, activation_fn, name='b_conv2')
            self.b_conv3, self.behaviour_weights['conv3_w'], self.behaviour_weights['conv3_b'] = conv2d(self.b_conv2,
                                        64, [3, 3], [1, 1], initializer, activation_fn, name='b_conv3')
            self.b_conv3_flat=tf.contrib.layers.flatten(self.b_conv3)

            self.b_fc1, self.behaviour_weights['fc1_w'], self.behaviour_weights['fc1_b'] = linear(self.b_conv3_flat, 512,
                                               activation_fn=activation_fn, name='b_fc1')
            self.b_out, self.behaviour_weights['out_w'], self.behaviour_weights['out_b'] = linear(self.b_fc1, num_outputs,
                                                                                                  name='b_out')
        ...
        with tf.variable_scope('target'):
            ...
            self.t_conv1, self.target_weights['conv1_w'], self.target_weights['conv1_b'] = conv2d(self.t_x,
                                                32, [8, 8], [4, 4], initializer, activation_fn, name='t_conv1')
            self.t_conv2, self.target_weights['conv2_w'], self.target_weights['conv2_b'] = conv2d(self.t_conv1,
                                     64, [4, 4], [2, 2], initializer, activation_fn,name='t_conv2')
            self.t_conv3, self.target_weights['conv3_w'], self.target_weights['conv3_b'] = conv2d(self.t_conv2,
                                     64, [3, 3], [1, 1], initializer, activation_fn, name='t_conv3')
            self.t_conv3_flat = tf.contrib.layers.flatten(self.t_conv3)
            self.t_fc1, self.target_weights['fc1_w'], self.target_weights['fc1_b'] = linear(self.t_conv3_flat, 512,
                                     activation_fn=activation_fn,name='t_fc1')
